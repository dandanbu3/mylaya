...
class BarScene extends Laya.Sprite {
    ...
    createPrizeCount () {
        this._prizeIcon = new Laya.Sprite('icons/gift_bg.png');
    ...
    createMileage () {
        this._mileageIcon = new Laya.Sprite('icons/mileage_bg.png');
    ...
    createPause () {
        this._pause = new Laya.Sprite('icons/btn_pause.png');
        this._pause.pivot(0);
        this._pause.pos(658, 206);
        // this._pause.setEventEnabled(true);
        this._pause.tap = (event) => {
        ...
        if (newValue.length < oldValue.length) {
            ...
            newArr.forEach((item, index) => {
                if (item !== oldArr[index]) {
                    const newSprite = new Laya.Sprite(`num/sm_${item}.png`);
